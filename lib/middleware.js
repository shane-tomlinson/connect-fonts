 ...
 *     require an Access-Control-allow_origin header.
 */
exports.setup = function(options) {
  ...
  fontPacks.forEach(function(fontPack) {
    ...
    for (var key in packFontConfig) {
      fontConfigs[key] = packFontConfig[key];

      for (var url in packFontConfig[key].urlToPaths) {
        urlToPaths[url] = packFontConfig[key].urlToPaths[url];
      }
    }
  });
