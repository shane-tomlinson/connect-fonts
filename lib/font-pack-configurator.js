...
function getSortedSubdirectories(root, locale_overrides) {
  var subdirs = {};
  ...
  // explicit overrides.
  for (var locale in locale_overrides) {
    subdirs[locale] = true;
  }

  // no default subdirectory, bunk.
  if (!subdirs['default']) throw new InvalidConfigError("Missing default font");
...
function addRemoteFormats(enabledFontTypes, fontName, root, locales, localeToSubdirs, formats, urlToPaths) {
  enabledFontTypes.forEach(function(extension) {
    ...
    for (var index = 0, locale; locale = locales[index]; ++index) {
      var url = format.url[locale] = "/fonts/" + locale + "/" + fontName + "."
                              + extension;

      // The actual location of the file on disk is what is specified in
      // localeToSubdirs. Use the locale override if available.
      if (localeToSubdirs[locale]) locale = localeToSubdirs[locale];

      var fontPath = urlToPaths[url] = path.join(root, locale, fontName + "." + extension);

      // Check whether the font file exists. If not, wah wah.
      if (!fs.existsSync(fontPath)) throw new InvalidConfigError("Missing font file: " + fontPath);
    }
 ...
 * @throws InvalidConfigError - thrown if:
 *          a) no default directory exists AND no default is specified in
 *                locale-to-subdirs
 *          b) a font file is missing
 */
module.exports = function(config) {
  ...
  var subdirs = getSortedSubdirectories(root, localeToSubdirs);
  ...
  for (var fontName in fonts) {
    ...
    addRemoteFormats(enabledFontTypes, fontName, root, subdirs,
        localeToSubdirs, outputFontConfig.formats, outputFontConfig.urlToPaths);
